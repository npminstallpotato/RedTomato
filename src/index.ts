...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case "--model":
        ...
        break;
      case "--effort":
        parsed.effort = args[++i] as ClaudeArgs["effort"];
        break;
      case "--permission-mode":
        parsed.permissionMode = args[++i];
        break;
      case "--max-budget-usd":
        parsed.maxBudgetUsd = Number(args[++i]);
        break;
      case "--stream":
        stream = true;
        break;
  ...
  if (!prompt) {
    console.error(
      "Usage: tsx src/index.ts [--prompt] <prompt> [options]\n\n" +
      "Options:\n" +
      "  --model <name>        Claude model (e.g., sonnet, opus)\n" +
      "  --effort <level>      low | medium | high | xhigh | max\n" +
      "  --permission-mode <m> auto | default | bypassPermissions\n" +
      "  --max-budget-usd <n>  Max API spend in USD\n" +
      "  --stream              Stream output in realtime"
    );
    process.exit(1);
  }

  if (stream) {
    for await (const chunk of claudeStream(prompt, parsed)) {
      if (typeof chunk === "string") process.stdout.write(chunk);
    }
    console.log();
  } else {
    const result = await claude(prompt, parsed);
